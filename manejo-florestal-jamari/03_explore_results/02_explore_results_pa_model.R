...
plotGradient(m, Gradient, pred=predY, measure="Y", index=15, las=1,
             ...
             showPosteriorSupport = FALSE)
# save as jpeg
jpeg(here("manejo-florestal-jamari", "results", "Dasypus_pa_vs_intensity.jpg")) # Open jpeg file
plotGradient(m, Gradient, pred=predY, measure="Y", index=15, las=1,
             showData = TRUE, main='occurrence (measure="Y")',
             xlab = "Logging intensity", ylab = "Dasypus (abundance)",
             showPosteriorSupport = FALSE)
dev.off()
...
# save as jpeg
jpeg(here("manejo-florestal-jamari", "results", "VP_pa.jpg"), width = 1200, height = 600) # Open jpeg file
plotVariancePartitioning(m, VP) 
dev.off()

# test: arrange species according to desired sequence
str(VP$vals)
VP_2 <- tibble(data.frame(VP$vals))
# in progress...
#%>% select(order(colnames(VP_2)))
...
# R2 averaged across species
mean(predY)
# R2 for individual species
R2_per_sp <- apply(predY, 2, mean)
R2_per_sp <- tibble(species = names(R2_per_sp), mean_R2 = R2_per_sp) %>%
