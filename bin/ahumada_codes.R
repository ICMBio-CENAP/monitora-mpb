...
# code modified by Elildo Carvalho Jr on 2021-10-05

# This code should process WILD.ID images
require(TeachingDemos)
#script to process raw TEAM files and get them ready for analysis
f.readin.fix.data<-function(filename){
	require(lubridate)
...
f.matrix.creator2<-function(data){
require(dplyr)        
  #results object
  res<-list()
...
# This function converts all the dates into date objects
...
#create new variable with binomial - genus species
	...
	data <- data.frame(data,bin=bin,timestamp=time.date)
...
#Function to transform a date time object yyyy-mm-dd hh:mm:ss to a decimal day object (0-1). Modified from code written by Brian
f.timeformatfunc=function(timedata){
        require(lubridate)
        ((hour(timedata)*60*60 + minute(timedata)*60 + second(timedata))/86400)*2*pi
}

f.extracthms <- function(datetimeob) {
        require(lubridate)
        new_hms <- paste(hour(datetimeob),minute(datetimeob), second(datetimeob), sep=":")
        hms(new_hms)
}
